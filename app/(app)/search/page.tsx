...
export default function SearchPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      data.forEach((stop, i) => idx.add(i, `${stop.name} ${stop.code} ${stop.id}`));
      setIndex(idx);
      setLoading(false);
    })();
  }, []);
  ...
  function handleQuickAdd(stopId: string) {
    if (!userId) return;
    if (!isSaved(stopId)) add(userId, stopId);
  }
  ...
  return (
    <div className="px-5 pt-10 pb-4">
      <h1 className="text-3xl font-bold tracking-tight mb-6">Find Stops</h1>
      ...
      {loading ? (
        ...
        <div>
          <h2 className="text-sm font-semibold text-text-secondary mb-3 px-1">Near you</h2>
          {nearbyLoading ? (
            <div className="space-y-3">
              {[...Array(5)].map((_, i) => <Skeleton key={i} className="h-16" />)}
            </div>
          ) : nearby.length === 0 ? (
            <EmptyState title="No stops nearby" description="No TransLink stops found near your location." />
          ) : (
            <div className="space-y-2">
              {nearby.map((stop) => (
                <StopResultRow
...
function StopResultRow({
  ...
  return (
    <Card className="p-3 flex items-center gap-3 active:scale-[0.98] transition-transform duration-200">
